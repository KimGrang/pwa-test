...
interface ServiceWorkerSupport {
...
export const useServiceWorker = () => {
  ...
  const [isSupported, setIsSupported] = useState<ServiceWorkerSupport>({
    serviceWorker: false,
...
// console.log('Periodic Sync 등록 성공');
   ...
   */
  const subscribeToPushNotifications = async (vapidPublicKey: string): Promise<PushSubscription | null> => {
...
// console.log('알림 권한:', permission);
   ...
   * @param options - 알림 옵션
   */
  const showLocalNotification = (title: string, options: NotificationOptions = {}): Notification => {
    if (!isSupported.pushNotifications) {
      throw new Error('Push Notifications가 지원되지 않습니다.');
    }
